...
type ChaosMonkeyConfigurationLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ChaosMonkeyConfiguration, err error)
...
// List lists all ChaosMonkeyConfigurations in the indexer.
func (s *chaosMonkeyConfigurationLister) List(selector labels.Selector) (ret []*v1alpha1.ChaosMonkeyConfiguration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ChaosMonkeyConfiguration))
	})
	return ret, err
}
...
type ChaosMonkeyConfigurationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ChaosMonkeyConfiguration, error)
...
// List lists all ChaosMonkeyConfigurations in the indexer for a given namespace.
func (s chaosMonkeyConfigurationNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ChaosMonkeyConfiguration, err error) {
...
// Get retrieves the ChaosMonkeyConfiguration from the indexer for a given namespace and name.
func (s chaosMonkeyConfigurationNamespaceLister) Get(name string) (*v1alpha1.ChaosMonkeyConfiguration, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("chaosmonkeyconfiguration"), name)
	}
	return obj.(*v1alpha1.ChaosMonkeyConfiguration), nil
}
